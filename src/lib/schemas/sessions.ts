...
export const createSessionSchema = z.object({
  eventId: z.string().min(1, "Event ID is required"),
  title: z.string().min(1, "Session title is required").max(255, "Title must be less than 255 characters"),
  description: z.string().max(5000, "Description must be less than 5000 characters").optional(),
  startAt: z.string().min(1, "Start time is required"),
  endAt: z.string().min(1, "End time is required"),
  roomId: z.number().int().min(1).nullable().optional(),
  chairId: z.string().min(1).nullable().optional(),
  meetingLink: z.string().url("Must be a valid URL").max(500, "Meeting link must be less than 500 characters").nullable().optional(),
}).refine((data) => {
...
export const updateSessionSchema = z.object({
  ...
  title: z.string().min(1, "Session title is required").max(255, "Title must be less than 255 characters").optional(),
