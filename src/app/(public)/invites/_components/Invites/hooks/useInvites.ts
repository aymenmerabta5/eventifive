...
export function useInvites() {
	...
	const {
		...
		refetch,
		isRefetching,
	} = useQuery(orpc.events.listMyInvites.queryOptions());
	...
	const { mutate: acceptReviewer, isPending: isAcceptingReviewer } = useMutation(
	...
	const { mutate: rejectReviewer, isPending: isRejectingReviewer } = useMutation(
	...
	const isMutating = isAcceptingSpeaker || isRejectingSpeaker || isAcceptingReviewer || isRejectingReviewer;
	...
	const handleAcceptSpeaker = useCallback((eventId: string) => {
		acceptSpeaker({ eventId });
	}, [acceptSpeaker]);

	const handleRejectSpeaker = useCallback((eventId: string) => {
		rejectSpeaker({ eventId });
	}, [rejectSpeaker]);

	const handleAcceptReviewer = useCallback((eventId: string) => {
		acceptReviewer({ eventId });
	}, [acceptReviewer]);

	const handleRejectReviewer = useCallback((eventId: string) => {
