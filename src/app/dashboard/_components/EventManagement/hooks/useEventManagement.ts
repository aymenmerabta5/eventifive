...
export function useEventManagement() {
  ...
  const organizerIds = useMemo(() => Array.from(new Set(events.map((e: any) => e.organizerId))), [events]);

  const organizerQueries = useQueries({
    queries: organizerIds.map((id: string) => orpc.profile.get.queryOptions({ input: { userId: id } })),
  });
  ...
  const enriched = events.map((e: any) => ({ ...e, organizerName: organizerMap[e.organizerId] ?? undefined })) as AdminEvent[];
  ...
  const stats: EventStats = {
    ...
    upcoming: enriched.filter((e) => new Date(e.startDate).getTime() > Date.now()).length,
    past: enriched.filter((e) => new Date(e.endDate).getTime() < Date.now()).length,
  ...
  return {
    ...
    refresh: () => void queryClient.invalidateQueries({ queryKey: orpc.events.adminList.queryOptions({ input: {} }).queryKey }),
  };
}
